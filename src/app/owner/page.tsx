...
export default async function OwnerDashboard() {
  ...
  const earnings = bookings.filter(x => x.paymentStatus === "PAID").reduce((sum, x) => sum + x.ownerPayout, 0);
  return <section className="section page"><div className="sectionhead"><div><span className="eyebrow">OWNER DASHBOARD</span><h1>Your business</h1><RedirectButton url="/api/stripe/connect" label={user.stripePayoutsEnabled ? "Manage Stripe payouts" : "Set up Stripe payouts"} className="outline"/></div><Link className="button" href="/owner/new">+ New listing</Link></div>
    <div className="stats"><div><b>${earnings.toLocaleString()}</b><span>paid earnings</span></div><div><b>{bookings.filter(x => x.status === "PENDING").length}</b><span>pending requests</span></div><div><b>{listings.length}</b><span>listings</span></div></div>
    <h2>Booking requests</h2><div className="rows">{bookings.map(booking => <div className="row" key={booking.id}><img src={booking.listing.photos[0]?.url} alt=""/><div><span className={`status ${booking.status.toLowerCase()}`}>{booking.status}</span> <span className="pill">{booking.paymentStatus}</span><h3>{booking.listing.title} · {booking.renter.name}</h3><p>{booking.startDate.toLocaleDateString()} — {booking.endDate.toLocaleDateString()} · ${booking.totalPrice}</p><div className="actions">{booking.status === "PENDING" && <><ApiButton url={`/api/bookings/${booking.id}`} method="PATCH" body={{ status: "ACCEPTED" }} label="Accept"/><ApiButton url={`/api/bookings/${booking.id}`} method="PATCH" body={{ status: "DECLINED" }} label="Decline" className="outline"/></>}<Link href={`/messages?with=${booking.renterId}`}>Message renter →</Link></div></div></div>)}{!bookings.length && <div className="empty"><h3>No requests yet</h3><p>Booking requests will appear here as travelers discover your listings.</p></div>}</div>
