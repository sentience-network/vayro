...
export async function getUser(){ try { const token=(await cookies()).get("vayro_session")?.value; if(!token)return null; const {payload}=await jwtVerify(token,key()); return await db.user.findUnique({where:{id:String(payload.userId)},select:{id:true,email:true,name:true,role:true,isOwner:true,disabled:true,subscriptionStatus:true,subscriptionEndsAt:true,stripeConnectAccountId:true,stripeChargesEnabled:true,stripePayoutsEnabled:true,identityVerificationStatus:true,identityVerifiedAt:true}}); } catch { return null; } }
