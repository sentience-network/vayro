...
export function ApiButton({ url, body, label, method = "POST", className = "button", confirmText }: { url: string; body?: object; label: string; method?: string; className?: string; confirmText?: string }) {
  ...
  }} type="button" aria-busy={busy}>{busy ? "Working…" : label}</button>;
}
...
export function BookingForm({ listingId, price, feePayer="RENTER", feeRate=10,representative=false }: { listingId: string; price: number;feePayer?:"RENTER"|"OWNER"|"SPLIT";feeRate?:number;representative?:boolean }) {
  ...
  return <form className="stack" onSubmit={async event => { event.preventDefault(); setError(""); const fields=Object.fromEntries(new FormData(event.currentTarget));try { await jsonFetch("/api/bookings", "POST", { ...fields, listingId,representativePhotoAccepted:fields.representativePhotoAccepted==="on" }); router.push("/dashboard/trips?booked=1"); } catch (e) { setError((e as Error).message); } }}><h3>${price} <small>/ day</small></h3>{representative&&<div className="representativeack"><b>⚠ No actual vehicle photos</b><p>Vayro is showing a make/model representation. Inspect the actual vehicle and documents before accepting possession.</p><label className="check"><input name="representativePhotoAccepted" type="checkbox" required/> I understand this is not a photo of the listed vehicle.</label></div>}<div className="dategrid"><label>Start<input name="startDate" type="date" min={today} value={start} onChange={event=>{setStart(event.target.value);if(end&&end<=event.target.value)setEnd("")}} required/></label><label>End<input name="endDate" type="date" min={start||today} value={end} onChange={event=>setEnd(event.target.value)} required/></label></div>{days>0&&<div className="quote"><span>Rental subtotal</span><b>${subtotal.toLocaleString()}</b><span>Marketplace fee ({feeRate}%)</span><b>{renterFee?`$${renterFee.toLocaleString()}`:"Owner-paid"}</b><span>Total due after acceptance</span><b>${total.toLocaleString()}</b><small>{feePayer==="SPLIT"?"Owner and renter split the marketplace fee.":feePayer==="OWNER"?"The owner chose to absorb the marketplace fee.":"The owner chose a renter-paid marketplace fee."}</small></div>}<label>Note<textarea name="note" placeholder="Tell the owner about your trip"/></label>{error && <p className="error" role="alert">{error}</p>}<button className="button">Request to book</button><small>No charge yet. The owner reviews your request.</small></form>;
}

export function ShareButton({title}:{title:string}){const[done,setDone]=useState(false),[error,setError]=useState("");return <><button type="button" className="outline" onClick={async()=>{setError("");try{const data={title,text:`Check out ${title} on Vayro`,url:window.location.href};if(navigator.share)await navigator.share(data);else if(navigator.clipboard){await navigator.clipboard.writeText(window.location.href);setDone(true);setTimeout(()=>setDone(false),1800)}else throw new Error("Sharing is not available in this browser") }catch(e){if((e as Error).name!=="AbortError")setError((e as Error).message)}}}>{done?"Link copied ✓":"Share ↗"}</button>{error&&<small className="error" role="alert">{error}</small>}</>}
